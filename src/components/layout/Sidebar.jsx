...
import {Briefcase, Database, Send} from "lucide-react";
import {useLocation} from "react-router-dom";
import {Drawer, List, ListItem, ListItemIcon, ListItemText, IconButton} from '@mui/material';

const menus = [
    {name: "داشبورد", link: "/", icon: <MdOutlineDashboard className="w-5 h-5"/>},
    {name: "کسب و کار", link: "/Business", icon: <Briefcase className="w-5 h-5"/>},
    {name: "دیتای هوشمند", link: "/smart-data", icon: <Database className="w-5 h-5"/>},
    {name: "درخواست ها", link: "/requests", icon: <Send className="w-5 h-5"/>},
        ...
        <>
            ...
            <section className="bg-primaryGray z-40 hidden 2xl:flex relative">
                <div className={`relative ${open ? "w-72" : "w-[4.5rem]"} duration-200`}>
                    <div
                        className={`fixed top-0 bottom-0 right-0 px-5 overflow-y-auto no-scrollbar ${open ? "w-72" : "w-fit"} max-h-screen`}>
